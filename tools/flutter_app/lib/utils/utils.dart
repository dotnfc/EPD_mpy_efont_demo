...
class StreamControllerReemit<T> {
  ...
  Stream<T> get stream {
    if (_latestValue == null) {
      ...
      return _controller.stream.newStreamWithInitialValue(_latestValue as T);
    }
  }
...
// "123.4" -> 12340 -> [0x00, 0x00, 0x30, 0x34]
List<int> convertIntToHexList(String sVal) {
  
  double multipliedValue = double.parse(sVal) * 100;
  int intValue = multipliedValue.toInt();

  List<int> hexList = List<int>.filled(4, 0, growable: false);

  for (int i = 3; i >= 0; i--) {
    hexList[i] = intValue & 0xFF;
    intValue >>= 8;
  }
